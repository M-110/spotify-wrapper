def validate_type(value,  param_name: str, expected_type):
    """Valid value is the same as expected_type."""
    if not isinstance(value, expected_type):
        raise TypeError(f'{param_name!r} must be of type {expected_type}')


def validate_range(num: int, param_name: str, min_: int, max_: int,):
    """Validate the num is between the inclusive range of min_ and max_."""
    if not ((num >= min_) and (num <= max_)):
        raise ValueError(f'{param_name!r} must be in the range of  {min_} to {max_}')


def validate_country(country_code: str, param_name: str):
    """Validate the country code is a valid 2 letter country code."""
    valid_codes = ['AF', 'AL', 'DZ', 'AS', 'AD', 'AO', 'AI', 'AQ', 'AG', 'AR', 'AM', 'AW', 'AU',
    ...
    if country_code.upper() not in valid_codes:
        raise ValueError(f'{country_code!r} is not a valid 2 letter country code. '
